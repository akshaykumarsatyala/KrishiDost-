...
Smart Farming Platform for Andhra Pradesh & Telangana. AI-powered crop disease detection, mandi price intelligence, crop recommendation, govt schemes, insurance claim generation, and pest outbreak heatmap. Bilingual: English + Telugu.
...
Because this app uses in-browser Babel (no bundler), modules are loaded as sequential <script type="text/babel"> tags in index.html. Variables declared in earlier scripts are available as globals to later scripts — no import/export needed.

Load order (from index.html):

Data modules (constants → treatments → priceData → cropData → schemesData → pestData → weatherData)
Shared components (Toast → Nav → WeatherWidget → Chatbot → Spark)
Page components (Home → DiseaseDetector → Prices → … → PestMap)
Root App component (inline in index.html)
To convert to a real module system (Vite/webpack), add export to each function/constant and import them in the consuming files.
...
Running the App
Open index.html directly in a browser — no build step needed. For the AI features (disease prediction, chatbot) to use live data, run the backend at http://localhost:8000.
